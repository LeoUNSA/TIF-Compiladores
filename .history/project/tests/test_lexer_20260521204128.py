...
def token_sig(source: str):
    """Retorna (type, literal) por token, sin posición — útil para comparar
    contenido ignorando dónde aparece el token en la fuente."""
    return [(t.type, t.literal) for t in lex(source)]
...
class TestComments:
    def test_line_comment_ignored(self):
        assert token_sig("// este comentario es ignorado\n42") == token_sig("42")

    def test_block_comment_ignored(self):
        assert token_sig("/* comentario */ 42") == token_sig("42")
