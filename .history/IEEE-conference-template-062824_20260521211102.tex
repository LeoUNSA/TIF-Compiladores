...
\begin{itemize}
    \item \texttt{-{}-explain-function <nombre>}: descripción en lenguaje natural de qué hace la función, sus parámetros y su tipo de retorno.
    \item \texttt{-{}-describe <variable>}: alcance, tipo y contexto de uso de una variable.
    \item \texttt{-{}-explain-error}: interpretación accesible de cada error léxico o semántico, con indicación de línea y causa probable.
    \item \texttt{-{}-navigate}: listado jerárquico de funciones, bloques y sentencias del programa.
\end{itemize}
...
\section{Avance Actual y Reflexiones Preliminares}
...
Las fases pendientes —análisis semántico, tabla de símbolos, CFG, capa LLM e interfaz de consulta— son las que determinarán si la hipótesis central se sostiene: que anclar un LLM en representaciones formales del compilador mejora la calidad y la confiabilidad de las explicaciones accesibles respecto a sistemas que operan sobre texto crudo. Esa validación es el objetivo del trabajo completo.
